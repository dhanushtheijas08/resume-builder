      ...
      <div className="container mx-auto">
        ...
        >
          ...
          <div className="relative flex flex-col md:flex-row gap-12">
            <div className="flex flex-col gap-8 px-8 py-12 md:px-16 md:py-20">
              <div className="flex flex-col gap-4">
                <motion.span
                  ...
                  className="text-primary-foreground/80 font-semibold tracking-wider text-sm"
                >
                  READY TO GET HIRED?
                </motion.span>
                <motion.h2
                  ...
                  className="text-4xl md:text-5xl text-nowrap font-bold text-primary-foreground leading-[1.1]"
                >
              ...
              >
                <Button
                  size="lg"
                  className="h-14 px-8 text-xl font-bold w-full"
                  asChild
                >
                  <Link href="/register">
            ...
            <div className="max-h-[480px] overflow-y-hidden rounded-t-xl shadow-2xl mt-18 max-w-4xl translate-x-5 w-full mx-auto">
              <ResumePreview />
            </div>
          </div>
        </motion.div>
    ...
    <div className="bg-white min-h-screen py-12 px-4 sm:px-8  leading-tight text-slate-900 max-w-4xl mx-auto">
      <div className="max-w-4xl mx-auto">
        ...
        <header className="text-center mb-8">
          <h1 className="text-4xl font-light tracking-tighter mb-2">{data.personal.name}</h1>
          <div className="flex justify-center flex-nowrap overflow-x-hidden gap-x-4 text-sm text-slate-600">
            <span>{data.personal.phone}</span>
            <span>•</span>
            <span className="font-semibold underline uppercase tracking-tight">{data.personal.email}</span>
            {data.personal.links.map((link, i) => (
              <React.Fragment key={i}>
                <span>•</span>
                <span className="hover:text-blue-600 cursor-pointer">{link}</span>
              </React.Fragment>
            ))}
          </div>
        </header>
        ...
        <section className="mb-8">
          <h2 className="text-sm font-bold border-b border-slate-800 mb-3 uppercase tracking-widest">Technical Skills</h2>
          <div className="text-sm space-y-1">
            <p><strong>Languages:</strong> {data.skills.languages}</p>
            <p><strong>Frameworks/Libraries:</strong> {data.skills.frameworks}</p>
            <p><strong>Cloud/DevOps:</strong> {data.skills.infrastructure}</p>
        ...
        <section className="mb-8">
          <h2 className="text-sm font-bold border-b border-slate-800 mb-4 uppercase tracking-widest">Professional Experience</h2>
        ...
        {/* Projects Section */}
        <section className="mb-8">
          <h2 className="text-sm font-bold border-b border-slate-800 mb-4 uppercase tracking-widest">Selected Projects</h2>
          <div className="grid gap-4">
            {data.projects.map((proj, i) => (
              <div key={i} className="text-sm">
                <span className="font-bold">{proj.name}</span> | <span className="text-slate-600 italic">{proj.desc}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Education Section */}
        <section>
          <h2 className="text-sm font-bold border-b border-slate-800 mb-3 uppercase tracking-widest">Education</h2>
